...
# hyperparams
batch_size = 32 # how many independent sequenceswill we process in parallel
...
# all the unique chars that occur in text
...
n = int(0.9*len(data)) # first 90% train, rest is validation
train_data = data[:n]
val_data = data[n:]
...
# everything in this function will not use .backward() cause of .no_grad(), efficiency
@torch.no_grad()
def estimate_loss():
    out = {}
    model.eval()
